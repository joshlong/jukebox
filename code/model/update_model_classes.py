...
def fixId(fn) :
   ...
   public void setDateModified(Date dc) { this.dateModified =dc; }
    ...
    contentOfFile = re.sub(r'public Set<(.*?)>',
                           r' @org.hibernate.annotations.Cache(usage = org.hibernate.annotations.CacheConcurrencyStrategy.READ_WRITE) public Set<\1> '
                           , contentOfFile)
    contentOfFile = re.sub(r'public class (.*?)',
                           r'@org.hibernate.annotations.Cache(usage = org.hibernate.annotations.CacheConcurrencyStrategy.READ_WRITE) public class \1'
                           ,contentOfFile)
    ...
    contentOfFile = re.sub( '@Id.*?@Column' ,
                            '@Id    @javax.persistence.GeneratedValue(strategy = javax.persistence.GenerationType.AUTO) @Column'
                            , contentOfFile)
...
if __name__ == '__main__':
    ...
    for path, dirs, files in os.walk( 'target'):
        for javaFile in [os.path.abspath(os.path.join(path, filename)) for filename in files if
                         fnmatch.fnmatch(filename, '*.java')]:
