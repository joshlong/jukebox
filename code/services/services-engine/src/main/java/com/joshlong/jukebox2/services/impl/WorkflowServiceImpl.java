...
import com.quietriot.utils.LoggingUtils;
...
public class WorkflowServiceImpl extends BaseService implements WorkflowService {
    ...
    private Collection<Long> _processInstances(
        ...
        List ids = getHibernateTemplate().executeFind(new HibernateCallback<List<Long>>() {
            public List<Long> doInHibernate(Session session) throws HibernateException, SQLException {
                ...
                if (criteria != null && criteria.keySet().size() > 0) {
                    ...
                    String q1 = " ( pi.rootToken.id IN ( select li.token.id from LongInstance li WHERE li.value = %s AND li.name =  %s ))  ",
                            q2 = "  ( pi.rootToken.id IN ( select si.token.id from StringInstance si WHERE si.value = %s   AND si.name =  %s )) ";
                    ...
                    for (String key : criteria.keySet()) {
                        ...
                        if (val instanceof Number) whichQuery = q1;
                        if (val instanceof String) whichQuery = q2;
                ...
                ids = getHibernateTemplate().findByNamedParam(hql, paramNames, paramVals);
    ...
    public Collection<Long> getOpenTaskInstancesByProcessInstanceAndActorAndCriteria(
        ...
        if (c1 != null)
            criteria.putAll(c1); // this way, its safe to assume the variable will never be null
...
//                TaskInstance taskInstance ;taskInstance.getToken().getProcessInstance().getId()
                ...
                if (criteria != null && criteria.keySet().size() > 0) {
                    ...
                    String q1 = "  ti.token.id IN ( select li.token.id from LongInstance li WHERE li.value = %s AND li.name =  %s )  ",
                            q2 = " ti.token.id IN ( select si.token.id from StringInstance si WHERE si.value = '%s' AND si.name =  %s ) ";
                    Collection<String> crits = new ArrayList<String>();
                    int ctr = 0;
                    for (String key : criteria.keySet()) {
                        Object val = criteria.get(key);
    ...
    public Collection<String> getActorsHavingOpenTaskInstances(String[] actors) {
        Collection<String> answer = (Collection<String>) getHibernateTemplate().findByNamedParam(
                " SELECT ti.actorId FROM TaskInstance ti WHERE ti.actorId IN( :tids )  and ti.isOpen = true and ti.start IS NULL and ti.isSuspended = false GROUP BY ti.actorId", "tids", actors);
        return answer;
    }
    ...
    public void startProcessInstance(final long processInstanceId) {
        jbpmTemplate.execute(new JbpmCallback() {
            public Object doInJbpm(JbpmContext jbpmContext) throws JbpmException {
                ...
                jmsTemplate.send(bpmsStartedPingDestination, new MessageCreator() {
    ...
    public Date createSchedulableDateFromDate(Date date) {
        ...
        if (DateUtils.round(date, hourOfDay).equals(nextHour))
            when = DateUtils.add(when, Calendar.MINUTE, 30); // thus, this will preserve every half hour
    ...
    public ProcessInstance scheduleProcessInstance(String flowName, Map<String, Object> vars, Date when) {
        ...
        vars2.put(WorkflowService.BPMS_PROCESS_INSTANCE_WELL_KNOWN_PROCESS_ATTRIBUTE_SCHEDULED_DATE, encodeDateForProcessAttribute(
                createSchedulableDateFromDate(
                        when)));
        vars2.putAll(vars);
    ...
    public Collection<Long> getProcessInstances(String pdName, Map<String, Object> vars) {
        Map<String, Object> vars2 = new HashMap<String, Object>();
        if (vars != null) vars2.putAll(vars);
    ...
    public void endProcessInstance(final Long processInstanceId) {
        ...
        getJbpmTemplate().execute(new JbpmCallback() {
            public Object doInJbpm(JbpmContext jbpmContext) throws JbpmException {
                loggingUtils.log("Ending %s", processInstanceId);
                ProcessInstance processInstance = jbpmContext.getProcessInstance(processInstanceId);
                processInstance.end();
                getJbpmTemplate().saveProcessInstance(processInstance);
    ...
    public void startScheduledProcessInstances(String pdName, Map<String, Object> vars, Date when) {
        ...
        {
            String dateAsString = encodeDateForProcessAttribute(
                    createSchedulableDateFromDate(when));

            loggingUtils.log("there are %s process instances scheduled to be started at %s", processInstances.size(), dateAsString);
    ...
    public int countOpenTaskInstances() {
        Number answer = (Number) getHibernateTemplate().find(" SELECT COUNT(ti.id) FROM TaskInstance ti WHERE   ti.isOpen = true and ti.isSuspended = false    ").iterator().next();
        return answer.intValue();
    }

    public int countOpenTaskInstancesForActors(String[] actors) {
        Number answer = (Number) getHibernateTemplate().findByNamedParam(" SELECT COUNT(ti.id) FROM TaskInstance ti WHERE ti.actorId IN( :tids )  and ti.isOpen = true and ti.isSuspended = false",
                "tids", actors).iterator().next();
        return answer.intValue();
    }
    ...
    public boolean lockTaskInstance(final long taskInstanceId, final String actor) {
        ...
        Boolean answer = (Boolean) getJbpmTemplate().execute(new JbpmCallback() {
            public Object doInJbpm(JbpmContext jbpmContext) throws JbpmException {
    ...
    /**
     * You give it a task instance and itll try and return the variable first
     * from the process scope * and then from the taskIntance scope. * * @param
     * taskInstanceId * @param key * @return
     */
